...
- Ask a friend with an iPhone to help
...
Ensure your `eas.json` is configured for production builds.

### Recommended `eas.json` for Production
...
After your app is live, you'll want to push updates when you improve features or fix bugs. Here's the workflow:

### Step 1: Make Your Code Changes
Edit your code as needed (bug fixes, new features, UI improvements, etc.).

### Step 2: Update Version Numbers
**CRITICAL**: You must increment version numbers for the stores to accept your update.
...
This creates new `.aab` (Android) and `.ipa` (iOS) files with your changes.
...
4. Add release notes (describe what's new/fixed)
...
1. Go to your app → **+ Version or Platform** → **iOS**
2. Enter new version number (must match `app.json`)
3. Add "What's New" text (release notes)
4. Select the build you just uploaded
5. **Save** → **Submit for Review**

### Step 5: User Updates
Once approved by the stores:
- **Android**: Users see "Update available" in Google Play Store
- **iOS**: Users see "Update" in App Store

Users must manually update unless they have auto-updates enabled on their device.
...
- [ ] Run `eas submit -p ios` and configure in App Store Connect (iOS)
- [ ] Write release notes
- [ ] Submit for review

**Timeline**: Android reviews take 1-3 days, iOS reviews take 1-7 days.
